...
# Include API routes
from routes import tasks, tags, chat, conversations
...
app.include_router(chat.router, dependencies=[Depends(get_current_user)])  # Apply JWT auth to chat routes
app.include_router(conversations.router, dependencies=[Depends(get_current_user)])  # Apply JWT auth to conversations routes
