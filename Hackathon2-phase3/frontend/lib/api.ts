...
export async function getConversations(userId: string): Promise<Conversation[]> {
  return apiRequest<Conversation[]>(`/api/${userId}/conversations`);
}

export async function getConversationMessages(userId: string, conversationId: string): Promise<ConversationMessage[]> {
  return apiRequest<ConversationMessage[]>(`/api/${userId}/conversations/${conversationId}/messages`);
}

export async function deleteConversation(userId: string, conversationId: string): Promise<void> {
  await apiRequest(`/api/${userId}/conversations/${conversationId}`, {
    method: 'DELETE',
  });
}
